...
export const graphConductor = (stavanger: GraphLayoutStavanger, configuration: ParserConfiguration): Epic  => {
    ...
    let initial: HortenItem = configuration.watcher ? stavanger[configuration.watcher] : stavanger.watcher
    ...
    const onPageInitLoadFlow = (action$, state$) =>
        action$.pipe(
            ...
            mergeMap(action => {
                return [
                    flow.model.fetchItem.request({data: {id: flowParamsAccessor(action.payload)}}),
                    initial.model.fetchItem.request({data: {id: watcherParamsAccessor(action.payload)}}),
                ]
            }));
    ...
    const onGraphAndSampleLoadedStartFlow = (action$, state$) =>
        action$.pipe(
            ofType(initial.model.fetchItem.success),
